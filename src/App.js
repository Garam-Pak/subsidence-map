...
const majorColorMap = {"타 지하시설물":"red", 상수:"grey", 하수:"blue", 오수:"green", 우수:"yellow", 지반:"orange", 맨홀:"violet", 기타:"grey"};
const colorCodes = { red: "#FF0000", blue: "#0000FF", green: "#008000", yellow: "#FFD700", orange: "#FFA500", violet: "#EE82EE", grey: "#808080" };

export default function App() {
  // 기기 타입 감지 (mobile < 600px, tablet < 1024px, desktop)
  ...
  return (
    <div className={`app-container ${device}`}>  {/* CSS 미디어쿼리로 flex-direction 제어 */}
      {/* 왼쪽 영역 (필터 + 지도) */}
        ...
        <MapContainer center={[36.5,127.8]} zoom={7} className="map-container">
          <LayersControl position="topright" collapsed={device==='mobile'}>
            ...
            <Overlay checked name="시군구 경계">
              {muniGeo && (
                <GeoJSON data={muniGeo} style={muniStyle} onEachFeature={(feat, layer)=>layer.bindTooltip(feat.properties.name, {sticky:true})}/>
              )}
            </Overlay>
          </LayersControl>

          {filteredData.map((m,i)=>(
            <Marker key={i} position={[parseFloat(m.latitude), parseFloat(m.longitude)]} icon={getIcon(m.category)} eventHandlers={{click:()=>setSelectedMarker(m)}} />
          ))}
        </MapContainer>
      </div>
