...
export default function App() {
  ...
  const icons = useMemo(() => {
    return Object.entries(iconUrls).reduce((acc, [k, url]) => {
      acc[k] = new L.Icon({ iconUrl: url, iconSize: device==='mobile'? [16,24]:[12,20], iconAnchor:device==='mobile'? [8,24]:[5,20], popupAnchor:[1,-20] });
      return acc;
    }, {});
  }, [device]);
        ...
        <div className="filter-bar">
          ...
          {[...majorCats,'기타'].map(m => (
            <div key={m} className="major-group">
              <button onClick={()=>setExpandedMajors(prev=>prev.includes(m)?prev.filter(x=>x!==m):[...prev,m])} className={expandedMajors.includes(m)?'expanded':''}>{m}</button>
              {expandedMajors.includes(m) && (grouped[m]||others).map(c=>(
                <button key={c} onClick={()=>setSelectedCategories(prev=>prev.includes(c)?prev.filter(x=>x!==c):[...prev,c])} className={selectedCategories.includes(c)?'selected':''}>{c}</button>
              ))}
            </div>
          ))}
          <label>검색:<input value={searchText} onChange={e=>setSearchText(e.target.value)} placeholder="하수,연약"/></label>
          <button onClick={()=>{const ws=XLSX.utils.json_to_sheet(filtered); const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'Data'); XLSX.writeFile(wb,'SinkholeData.xlsx');}}>Excel</button>
          <label><input type="checkbox" checked={showAging} onChange={e=>setShowAging(e.target.checked)}/> 노후화율</label>
          {showAging && <label>년도:
            <select value={selectedYear} onChange={e=>setSelectedYear(e.target.value)}>
              {[2022,2021,2020,2019,2018,2017,2016].map(y=><option key={y} value={y}>{y}</option>)}
            </select>
          </label>}
        </div>
        <MapContainer center={[36.5,127.8]} zoom={7} className="map-container">
          <LayersControl position="topright" collapsed={device==='mobile'}>
            <BaseLayer checked name="OSM"><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="© OpenStreetMap"/></BaseLayer>
            <BaseLayer name="ESRI Satellite"><TileLayer url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}" attribution="© ESRI"/></BaseLayer>
            <BaseLayer name="Google Map"><TileLayer url="http://{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}" subdomains={["mt0","mt1","mt2","mt3"]} attribution="© Google"/></BaseLayer>
            <BaseLayer name="Google Satellite"><TileLayer url="http://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}" subdomains={["mt0","mt1","mt2","mt3"]} attribution="© Google"/></BaseLayer>
            <Overlay checked name="시군구 경계">{muniGeo && <GeoJSON data={muniGeo} style={muniStyle} onEachFeature={(f,l)=>l.bindTooltip(f.properties.name,{sticky:true})}/>}</Overlay>
          </LayersControl>
          {filtered.map((d,i)=><Marker key={i} position={[+d.latitude,+d.longitude]} icon={icons[majorColorMap[majorCats.find(m=>d.category.startsWith(m))||'기타']]} eventHandlers={{click:()=>setSelectedMarker(d)}}/>) }
        </MapContainer>
      </div>
      <div className="dashboard-panel">
        {selectedMarker ? (
          <div>
            ...
            <ul><li>폭: {selectedMarker.width}m</li><li>연장: {selectedMarker.length}m</li><li>깊이: {selectedMarker.depth}m</li></ul>
          ...
          <div>
            ...
            <ResponsiveContainer width="100%" height={200}><BarChart data={chartData} layout="vertical"><XAxis type="number" hide/><YAxis type="category" dataKey="name" width={80}/><Tooltip/><Bar dataKey="value"/></BarChart></ResponsiveContainer>
          </div>
        )}
        <div className="legend">
          <h4>범례</h4>
          {Object.entries(majorColorMap).map(([k,v])=><div key={k}><span style={{background:v,width:10,height:10,display:'inline-block',marginRight:5}}/> {k}</div>)}
        </div>
      </div>
    </div>
  );
